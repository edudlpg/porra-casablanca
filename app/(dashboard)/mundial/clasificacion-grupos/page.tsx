    ...
    <div className="flex min-w-0 items-center gap-2">
      {displayFlagUrl ? (
        ...
        <img
          ...
          className="size-5 shrink-0 rounded-[6px] bg-white object-contain p-0.5"
        />
      ) : (
      ...
      <span className="truncate text-[13px] font-semibold tracking-[0.08em]">{getTeamCode(team.name)}</span>
    </div>
  );
}
      ...
      {standings.length ? (
        <div className="space-y-4">
              ...
              <Card key={group.groupCode}>
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between gap-3">
                    <CardTitle>Grupo {group.groupCode}</CardTitle>
                    <Badge variant={isFinalized ? "success" : "secondary"}>
                      {isFinalized ? "Finalizado" : "Abierto"}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[23.5rem] table-fixed border-separate border-spacing-y-1.5">
                      <thead>
                        <tr className="text-left text-[11px] font-bold uppercase tracking-[0.16em] text-slate-600">
                          <th className="w-[5.25rem] px-2 py-1">Equipo</th>
                          <th className="w-7 px-0.5 py-1 text-center">V</th>
                          <th className="w-7 px-0.5 py-1 text-center">E</th>
                          <th className="w-7 px-0.5 py-1 text-center">D</th>
                          <th className="w-8 px-0.5 py-1 text-center">GF</th>
                          <th className="w-8 px-0.5 py-1 text-center">GC</th>
                          <th className="w-8 px-0.5 py-1 text-center">PTS</th>
                        </tr>
                      </thead>
                      <tbody>
                        {group.rows.map((row, index) => (
                          <tr key={row.team.id} className={getGroupRowClass(index)}>
                            <td className="rounded-l-2xl max-w-0 px-2 py-2">
                              <CompactTeamCell team={row.team} />
                            </td>
                            <td className="px-0.5 py-2 text-center font-semibold">{row.wins}</td>
                            <td className="px-0.5 py-2 text-center font-semibold">{row.draws}</td>
                            <td className="px-0.5 py-2 text-center font-semibold">{row.losses}</td>
                            <td className="px-0.5 py-2 text-center font-semibold">{row.goalsFor}</td>
                            <td className="px-0.5 py-2 text-center font-semibold">{row.goalsAgainst}</td>
                            <td className="rounded-r-2xl px-0.5 py-2 text-center font-bold">{row.points}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          <Card>
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full min-w-[24rem] table-fixed border-separate border-spacing-y-1.5">
                  <thead>
                    <tr className="text-left text-[11px] font-bold uppercase tracking-[0.16em] text-slate-600">
                      <th className="w-[5.25rem] px-2 py-1">Equipo</th>
                      <th className="w-7 px-0.5 py-1 text-center">V</th>
